...
suite('Unit Tests', function () {
  ...
  suite('Comparisons', function () {
    ...
    test('#isAbove, #isAtMost', function () {
      assert.isAtMost('hello'.length, 5, 'expect length to be 5');
      assert.isAbove(1, 0, 'expect 1 to be greater than zero');
      assert.isAbove(Math.PI, 3, 'PI == 3.1415...');
    ...
    test('#approximately', function () {
      assert.approximately(weirdNumbers(0.5), 1, 0.5, 'expected to be < 1' );
      assert.approximately(weirdNumbers(0.2), 1, 0.2, 'expected to be < 1');
    });
  });
